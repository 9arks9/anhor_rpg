...
class FightModel:
    ...
    def run(self):
        logger.log('==================== FIGHT STARTED ====================','fight')
        ...
        while self.enemy_hp > 0 and self.hero_hp > 0:
            ...
            if h_dmg <= 0:
                logger.log(f'{self.hero.name} missed the attack.','fight')
            self.enemy_hp -= h_dmg
            ...
            if self.enemy_hp < 0:
                self.enemy_hp = 0
            logger.log(f'{self.hero.name} --dealt {h_dmg} damage to-- {self.enemy.name}.','fight')
            logger.log(f'{self.enemy.name} HP: {self.enemy_hp}/{self.enemy.hp}\n___________','fight')
            if self.enemy_hp <= 0:
                break
            # Enemy's turn
            e_dmg = self.damage_calculator(self.enemy_attack, self.hero_defence)
            if e_dmg <= 0:
                logger.log(f'{self.enemy.name} missed the attack.','fight')
            self.hero_hp -= e_dmg
            ...
            if self.hero_hp < 0:
                self.hero_hp = 0
            logger.log(f'{self.enemy.name} --dealt {e_dmg} damage to-- {self.hero.name}.','fight')
            logger.log(f'{self.hero.name} HP: {self.hero_hp}/{self.hero.hp}\n___________','fight')
        # Display Fight result
        if self.hero_hp <= 0:
            logger.log(f'{self.hero.name} is dead.','fight)
        elif self.enemy_hp <= 0:
            logger.log(f'{self.enemy.name} is dead.','fight')
            self.hero.add_exp(self.enemy.level * 5)
            self.enemy.add_death(1)
            logger log(f'{self.hero.name} gained {self.enemy.level * 5} experience points.','fight')
        else:
            logger.log('Draw','fight')
